...
function renderCamposPage() {
  ...
  slice.forEach(({ nombre: campo, tipo }) => {
    ...
    if (tipo === "largo") {
      div.innerHTML = `
        <label style="display:flex;align-items:center;gap:8px;">${esc(campo)}
      ...
      div.innerHTML = `<label>${esc(campo)}</label>
...
async function obtenerGratis() {
  if (!currentUser) { toast("Debes iniciar sesión para obtener el documento.", "error"); return; }
...
async function simularPago() {
  if (!currentUser) { toast("Debes iniciar sesión para continuar.", "error"); return; }
...
async function iniciarPagoWompi() {
  if (!currentUser) { toast("Debes iniciar sesión para realizar el pago.", "error"); return; }
...
/* ══════════════════════════════════════════════════════
   ...
   Estrategia: fusionar runs dentro de cada párrafo para
   reconstruir el texto completo, luego reemplazar y
   volver a escribir el párrafo con formato preservado.
   ══════════════════════════════════════════════════════ */

/* Fusiona todos los <w:r> de un párrafo en un único run
   para que el texto sea continuo y reemplazable.
   Preserva pPr (propiedades de párrafo) y rPr (formato del primer run). */
function fusionarRunsParrafo(parrafoXml) {
  const pPrMatch = parrafoXml.match(/<w:pPr>[\s\S]*?<\/w:pPr>/);
  const pPr = pPrMatch ? pPrMatch[0] : "";

  // Capturar todos los runs con su contenido
  const runs = [...parrafoXml.matchAll(/<w:r\b[^>]*>[\s\S]*?<\/w:r>/g)].map(m => m[0]);
  if (!runs.length) return { fusionado: parrafoXml, pPr, rPr: "", textoFusionado: "" };

  // Obtener rPr del primer run que lo tenga
  let rPr = "";
  for (const r of runs) {
    const rPrM = r.match(/<w:rPr>[\s\S]*?<\/w:rPr>/);
    if (rPrM) { rPr = rPrM[0]; break; }
  }

  // Concatenar el texto de todos los <w:t>
  const textoFusionado = runs.map(r => {
    const ts = [...r.matchAll(/<w:t[^>]*>([^<]*)<\/w:t>/g)];
    return ts.map(t => t[1]).join("");
  }).join("");

  return { pPr, rPr, textoFusionado };
}
...
async function reemplazarEnDocx(blob, campos) {
  ...
  // Ordenar campos de mayor a menor longitud para evitar reemplazos parciales incorrectos
  const entradasOrdenadas = Object.entries(campos).sort((a, b) => b[0].length - a[0].length);

  archivos.forEach(f => {
    if (!zip.files[f]) return;
    let xml = zip.files[f].asText();

    // ── Limpiar marcas de Word que fragmentan el texto entre runs
    xml = xml
      ...
      // Quitar wrappers de control de cambios (<w:ins>, <w:del>) pero conservar su contenido de runs
      .replace(/<w:del\b[^>]*>[\s\S]*?<\/w:del>/g, "")
      .replace(/<w:ins\b[^>]*>([\s\S]*?)<\/w:ins>/g, "$1")
      // Quitar wrappers de hipervínculos y campos simples, conservando los runs internos
      .replace(/<w:hyperlink\b[^>]*>([\s\S]*?)<\/w:hyperlink>/g, "$1")
      .replace(/<w:fldSimple\b[^>]*>([\s\S]*?)<\/w:fldSimple>/g, "$1")
      // Quitar instrucciones de campos (fldChar/instrText) que fragmentan runs
      .replace(/<w:fldChar\b[^>]*\/>/g, "")
      .replace(/<w:fldChar\b[^>]*>[\s\S]*?<\/w:fldChar>/g, "")
      .replace(/<w:instrText\b[^>]*>[\s\S]*?<\/w:instrText>/g, "");

    entradasOrdenadas.forEach(([campo, valor]) => {
      const campoNorm = campo.replace(/\s+/g, " ").trim();
      const campoUP   = campoNorm.toUpperCase();
      const valorStr  = String(valor);
      const tieneNewline = valorStr.includes("\n");
      let encontrado = false;

      // ── ESTRATEGIA 1: texto aparece completo en el XML (el caso más sencillo)
      if (!tieneNewline && xml.includes(campoNorm)) {
        xml = xml.split(campoNorm).join(xmlEsc(valorStr));
        console.log(`[DOCX E1] ✅ "${campoNorm}"`);
        encontrado = true;
      }

      // ── ESTRATEGIA 2: case-insensitive en el XML completo
      if (!encontrado && !tieneNewline) {
        const escaped = campoNorm.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const re = new RegExp(escaped, "gi");
        if (re.test(xml)) {
          re.lastIndex = 0;
          xml = xml.replace(re, xmlEsc(valorStr));
          console.log(`[DOCX E2] ✅ case-insensitive: "${campoNorm}"`);
          encontrado = true;
        }
      }

      // ── ESTRATEGIA 3a: texto multilinea (ej: salida de IA con PRIMERO/SEGUNDO/TERCERO)
      // Busca el párrafo completo <w:p>...</w:p> que contenga el placeholder
      // y lo reemplaza con múltiples párrafos Word. Usa regex directa — no split/join.
      if (!encontrado && tieneNewline) {
        let hitP = false;
        xml = xml.replace(/<w:p\b[^>]*>[\s\S]*?<\/w:p>/g, fullP => {
          const { pPr, rPr, textoFusionado } = fusionarRunsParrafo(fullP);
          if (!textoFusionado.toUpperCase().includes(campoUP)) return fullP;
          hitP = true;
          return textoAWordXml(valorStr, pPr, rPr);
        });
        if (hitP) {
          console.log(`[DOCX E3a] ✅ Multilinea: "${campoNorm}"`);
          encontrado = true;
        }
      }

      // ── ESTRATEGIA 3b: texto de una sola línea con runs fragmentados
      // El placeholder aparece partido entre varios <w:r> — se fusionan para encontrarlo.
      if (!encontrado && !tieneNewline) {
        let hitP = false;
        // Dividir en partes por </w:p>; el join("</w:p>") añade el cierre entre partes.
        // Las partes sin match se devuelven tal cual (el join pone su </w:p>).
        const partes = xml.split(/<\/w:p>/);
        const partesNuevas = partes.map((parte, idx) => {
          if (idx === partes.length - 1) return parte; // cierre del doc, sin párrafo
          const wPIdx = parte.lastIndexOf("<w:p");
          if (wPIdx === -1) return parte; // fragmento sin <w:p, el join pone </w:p>
          const antesParrafo = parte.substring(0, wPIdx);
          const parrafoCompleto = parte.substring(wPIdx);
          const { pPr, rPr, textoFusionado } = fusionarRunsParrafo(parrafoCompleto);
          if (!textoFusionado.toUpperCase().includes(campoUP)) return parte; // sin match
          hitP = true;
          const tagAperturaM = parrafoCompleto.match(/^<w:p\b[^>]*>/);
          const tagApertura = tagAperturaM ? tagAperturaM[0] : "<w:p>";
          const escapedCampo = campoUP.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
          const reP = new RegExp(escapedCampo, "gi");
          const textoNuevo = textoFusionado.replace(reP, xmlEsc(valorStr));
          // Sin </w:p> al final — el join("</w:p>") lo agrega
          return antesParrafo + `${tagApertura}${pPr}<w:r>${rPr}<w:t xml:space="preserve">${textoNuevo}</w:t></w:r>`;
        });
        const xmlNuevo = partesNuevas.join("</w:p>");
        if (hitP) {
          xml = xmlNuevo;
          console.log(`[DOCX E3b] ✅ Runs fusionados: "${campoNorm}"`);
          encontrado = true;
        }
      }

      // ── ESTRATEGIA 4: whitespace flexible en XML crudo (último recurso)
      if (!encontrado && !tieneNewline) {
        const escaped4 = campoNorm
          .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
          .replace(/ /g, "\\s+");
        const re4 = new RegExp(escaped4, "gi");
        if (re4.test(xml)) {
          re4.lastIndex = 0;
          xml = xml.replace(re4, xmlEsc(valorStr));
          console.log(`[DOCX E4] ✅ Whitespace flex: "${campoNorm}"`);
          encontrado = true;
        }
      }

      // ── ESTRATEGIA 5: fusión global de runs dentro de párrafos (maneja tablas y párrafos anidados)
      // Reemplaza directamente en el XML de cada párrafo <w:p>...</w:p> encontrado en cualquier contexto.
      if (!encontrado && !tieneNewline) {
        let hitP5 = false;
        xml = xml.replace(/<w:p[ >][\s\S]*?<\/w:p>/g, fullP => {
          const { pPr, rPr, textoFusionado } = fusionarRunsParrafo(fullP);
          if (!textoFusionado.toUpperCase().includes(campoUP)) return fullP;
          hitP5 = true;
          const escapedCampo5 = campoUP.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
          const reP5 = new RegExp(escapedCampo5, "gi");
          const textoNuevo5 = textoFusionado.replace(reP5, xmlEsc(valorStr));
          const tagAperturaM5 = fullP.match(/^<w:p\b[^>]*>/);
          const tagApertura5 = tagAperturaM5 ? tagAperturaM5[0] : "<w:p>";
          return `${tagApertura5}${pPr}<w:r>${rPr}<w:t xml:space="preserve">${textoNuevo5}</w:t></w:r></w:p>`;
        });
        if (hitP5) {
          console.log(`[DOCX E5] ✅ Fusión global párrafos (tabla/anidado): "${campoNorm}"`);
          encontrado = true;
        }
      }

      if (!encontrado) {
        console.warn(`[DOCX] ❌ "${campoNorm}" NO encontrado en ninguna estrategia`);
      }
    });
  ...
  minutasList.innerHTML = slice.map(m => {
    ...
    return `<div class="admin-item">
      <div>
        ...
        <br><small>$${Number(m.precio||0).toLocaleString("es-CO")} COP · ${(m.campos||[]).length} campos · ${m.docxBase64 ? "✅ Plantilla lista" : "❌ Sin plantilla"} · ${(m.docxPreviewURL || m.docxPreviewBase64) ? "✅ Preview" : "⚠️ Sin preview (verán la plantilla)"}</small>
      </div>
